...
function setupAlertInitialize() {
	// if OWF is running, then launch setup widget if it is not already opened
	if( OWF.Util.isRunningInOWF() ) {
			
		// launch the setup widget
		...
		widgetLauncher.launchWidget({
			universalName: 'ae84cdb3-2db0-b4b3-e0c3-5094c70fce55',
			guid: 'ae84cdb3-2db0-b4b3-e0c3-5094c70fce55',				
			title: 'ICODE-MDA Setup Alerts',
			launchOnlyIfClosed: true
		});
			
		// send rest of function as a string to setupAlerts Channel to be executed on maps
		var msg = "";
		msg = msg + "initializePolygon();";
		msg = msg + "google.maps.event.addListener(map, 'click', addPoint);";
		msg = msg + "$('#setup-alert-modal').bind('dialogclose', function(event) {";
		msg = msg + "google.maps.event.clearListeners(map, 'click');";
		msg = msg + "deletePolygon();";
		msg = msg + "$('#panel').toggle(true);";
		msg = msg + "$('#alertPanel').toggle(true);";
		msg = msg + "});";
		//OWF.Eventing.publish("setupAlerts_setupAlertInitialize", msg);
		...
		$('#setup-alert-modal').load('setup-alerts.html', function() {
			$("#dialog").dialog("open");
		})
		.dialog('open');
